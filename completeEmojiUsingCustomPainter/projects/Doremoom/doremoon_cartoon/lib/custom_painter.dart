...
class DoremoonPinter extends CustomPainter {
  ...
  void paint(canvas, Size size) {
    ...
    final leftEyecenter =
        Offset(width / 2 - Xeyedivation, height / 2 - Yeyedivation);
    final RighrtEyecenter =
        Offset(width / 2 + Xeyedivation, height / 2 - Yeyedivation);
    final Leftlenscenter =
        Offset(width / 2 - Xeyedivation, height / 2 - Yeyedivation);
    final Righrtlenscenter =
        Offset(width / 2 + Xeyedivation, height / 2 - Yeyedivation);
    final Leftinnerlenscenter =
        Offset(width / 2 - Xeyedivation, height / 2 - Yeyedivation);
    final Righrtinnerlenscenter =
        Offset(width / 2 + Xeyedivation, height / 2 - Yeyedivation);
    final NoseCenter = Offset(width / 2, height / 2 - nosedivation);
    final innerNoseCenter = Offset(width / 2, height / 2 - nosedivation);
    final nosestartline =
        Offset(NoseCenter.dx, NoseCenter.dy + nosestartdevation);
    final noseendline = Offset(mouthcenter.dx, mouthcenter.dy);

    // draw body
    final bodycenter = Offset(width / 2, headcenter.dy + bodydivation);
    final pocketbackcenter =
        Offset(bodycenter.dx, bodycenter.dy - pocketdivation);
    final pocketcenter = Offset(width / 2, bodycenter.dy - pocketYdivation);

    // draw legs
    final leftlegtstartline = Offset(
        pocketcenter.dx - legXivation, pocketcenter.dy + legYdivationstart);
    final leftlegendtline = Offset(
        pocketcenter.dx - legXivation, pocketcenter.dy + legYdivationend);
    final rightleftstartline = Offset(
        pocketcenter.dx + legXivation, pocketcenter.dy + legYdivationstart);
    final righttlegendtline = Offset(
        pocketcenter.dx + legXivation, pocketcenter.dy + legYdivationend);
    final leftfootcenter =
        Offset(leftlegendtline.dx, leftlegendtline.dy + footdivation);
    final righttfootcenter =
        Offset(righttlegendtline.dx, leftlegendtline.dy + footdivation);
    ...
    //  draw arms
    final leftarmstart =
        Offset(pocketcenter.dx + armXdivation, pocketcenter.dy - armYdivation);
    final leftarmend =
        Offset(pocketcenter.dx + xdivation, pocketcenter.dy - ydivation);
    final rightarmsart =
        Offset(pocketcenter.dx - armXdivation, pocketcenter.dy - armYdivation);
    final rightarmend =
        Offset(pocketcenter.dx - xdivation, pocketcenter.dy - ydivation);
    ...
    final Mustache1stratline =
        Offset(NoseCenter.dx - StartMustachesXderivation, NoseCenter.dy);
    final Mustache1endline = Offset(NoseCenter.dx - EndtMustachesXderivation,
        NoseCenter.dy + EndtMustachesYderivation);
    final Mustachestratline2 =
        Offset(headcenter.dx - StartMustachesXderivation, headcenter.dy);
    final Mustacheendline2 =
        Offset(headcenter.dx - EndtMustachesXderivation, headcenter.dy);
    final Mustachestratline3 = Offset(headcenter.dx - StartMustachesXderivation,
        headcenter.dy + rightMustachesydevation3);
    final Mustacheendline3 = Offset(headcenter.dx - EndtMustachesXderivation,
        headcenter.dy + EndtMustachesYderivation);

    // Draw Right Mustaches
    final RMustache1stratline =
        Offset(NoseCenter.dx + StartMustachesXderivation, NoseCenter.dy);
    final RMustache1endline = Offset(NoseCenter.dx + EndtMustachesXderivation,
        NoseCenter.dy + EndtMustachesYderivation);
    final RMustachestratline2 =
        Offset(headcenter.dx + StartMustachesXderivation, headcenter.dy);
    final RMustacheendline2 =
        Offset(headcenter.dx + EndtMustachesXderivation, headcenter.dy);
    final RMustachestratline3 = Offset(
        headcenter.dx + StartMustachesXderivation,
        headcenter.dy + rightMustachesydevation3);
    final RMustacheendline3 = Offset(headcenter.dx + EndtMustachesXderivation,
        headcenter.dy + EndtMustachesYderivation);
    ...
    // draw smily
    canvas.drawArc(Rect.fromCircle(center: mouthcenter, radius: mouthradius),
        degToRadians(0), degToRadians(180), true, nosepaint);
    //  left Eye
...
//  Draw  right Mustaches
    ...
    // draw pocket
    canvas.drawArc(Rect.fromCircle(center: pocketcenter, radius: pocketradius),
        degToRadians(0), degToRadians(180), true, pocketpaint);
    //  draw pocketborder
    canvas.drawArc(Rect.fromCircle(center: pocketcenter, radius: pocketradius),
        degToRadians(0), degToRadians(180), true, pocketborderpaint);
    //  draw belt
